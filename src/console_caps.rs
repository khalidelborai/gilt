...
impl ConsoleCapabilities {
    ...
    ///
    /// Reads `COLORTERM`, `TERM`, `UNICODE_VERSION`, and detects whether
    /// stdout is a terminal. This is the constructor called by
    /// `Console::from_builder`.
...
mod tests {
    ...
    fn caps_truecolor_flag_when_colorterm_24bit() {
        let caps = ConsoleCapabilities::from_env_parts(Some("24bit"), None, true, None);
    ...
    fn caps_no_truecolor_when_no_colorterm() {
        let caps = ConsoleCapabilities::from_env_parts(None, Some("xterm-256color"), true, None);
    ...
    fn caps_unicode_version_none_on_non_numeric() {
        let caps = ConsoleCapabilities::from_env_parts(None, None, false, Some("fifteen"));
    ...
    fn caps_is_terminal_propagated() {
        let tty = ConsoleCapabilities::from_env_parts(None, None, true, None);
        assert!(tty.is_terminal);
        let piped = ConsoleCapabilities::from_env_parts(None, None, false, None);
